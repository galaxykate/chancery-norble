...
class HeartBeatTime {
    ...
    constructor({timeSignature=24, maxDT=100,startTime, beatEvery=100})  {
    ...
    togglePause() {
        ...
        if (this.isPaused) {
            ...
            this.totalPausedTime += this.lastClockTime - this.pauseStart
        } else {
            // Start the pause
    ...
    update(t) {
        ...
        if (!this.isPaused) {
            ...
            // Ignore any paused time
            t -= this.totalPausedTime

            // Increase this amount of time
            this.dt = Math.min(t - this.t, this.maxDT)
            this.t += this.dt
